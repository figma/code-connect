...
export default { example: figma.code\`def python_code():
  ...
  describe('Legacy find methods', () => {
    describe('__findChildWithCriteria__', () => {
      it('should migrate TEXT type with __render__() to findText().__render__()', () => {
        const input = `const text = figma.selectedInstance.__findChildWithCriteria__({ name: 'Label', type: "TEXT" }).__render__()`
        const expected = `const text = figma.selectedInstance.findText('Label').__render__()`
        expect(migrateV1TemplateToV2(input)).toBe(expected)
      })
...
const r2 = instance2.executeTemplate().example`
      ...
      it('should preserve findText().__render__() and not rewrite it to executeTemplate()', () => {
        // The TEXT __findChildWithCriteria__ migration keeps `__render__()` (which
        // exists on both TextHandle and ErrorHandle) rather than rewriting it to
        // `executeTemplate().example` (which TextHandle does not implement). A
        // sentinel shields it from the blanket __render__ rewrite (step 8).
        const input = `const text = figma.selectedInstance.__findChildWithCriteria__({ name: 'Label', type: "TEXT" }).__render__()`
        const result = migrateV1TemplateToV2(input)
        expect(result).toBe(`const text = figma.selectedInstance.findText('Label').__render__()`)
        expect(result).not.toContain('executeTemplate')
        expect(result).not.toContain('__findChildWithCriteria__')
        // The sentinel must be fully resolved back to __render__()
        expect(result).not.toContain('__FCC_TEXT_RENDER__')
      })
    })
...
const p2 = instance2.executeTemplate().metadata.props`
  ...
  describe('Real-world generated patterns', () => {
    it('should migrate TextContent intrinsic pattern', () => {
      ...
      const expected = `figma.selectedInstance.findText('Label').__render__()`
      expect(migrateV1TemplateToV2(input)).toBe(expected)
    })
