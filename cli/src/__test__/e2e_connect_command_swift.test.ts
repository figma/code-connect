...
describe('e2e test for `connect` command (Swift .xcodeproj)', () => {
  const cliVersion = require('../../package.json').version
  it(
    'successfully parses Swift files',
    async () => {
      const testPath = path.join(__dirname, 'e2e_connect_command/swift_parser')
...
describe('e2e test for `connect` command (Swift Package.swift project)', () => {
  const cliVersion = require('../../package.json').version
  it(
    'successfully parses Swift files',
    async () => {
      const testPath = path.join(__dirname, 'e2e_connect_command/swift_package')
